...
// Use /tmp for writing data on Vercel (safe ephemeral storage that prevents Read-Only filesystem errors)
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(500).json({
        error: "GEMINI_API_KEY is not configured on the Vercel/Express server. Please add it to your Environment Variables.",
      });
    }

    // Lazy init clients to pick up dynamic env updates
    if (!ai) {
      ai = new GoogleGenAI({
        apiKey: process.env.GEMINI_API_KEY,
        httpOptions: {
...
// App needs to be exported for server.ts and Vercel
export default app;
